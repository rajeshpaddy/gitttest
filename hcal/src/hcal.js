...
(function ($) {
  ...
  function createHcalHeader(dates, markerpos, today, format) {
    var $header = $('<div class="hcal-header">'),
      ...
      $dateInner,
      $today,
      days = format === 'ger' ? ['M', 'D', 'M', 'D', 'F', 'S', 'S'] : ['M', 'T', 'W', 'T', 'F', 'S', 'S','M', 'T', 'W', 'T', 'F', 'S', 'S','M', 'T', 'W', 'T', 'F', 'S', 'S','M', 'T', 'W', 'T', 'F', 'S', 'S'],
      date;
    ...
    for (i = 0; i < dates.length; i = i + 1) {
      ...
      if (parseInt(markerpos[i])==1) {
        $dateInner.addClass('active');
      }
	if (parseInt(markerpos[i])==2) {
        $dateInner.addClass('special');
      }
    }
